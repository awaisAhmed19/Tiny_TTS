...
void write_wav(const WavFile &wav) {
  ...
  char chunk_id2[4] = {'d', 'a', 't', 'a'};
  uint32_t subchunk_size = 16;

  file.write(chunk_id1, 4);
  file.write(reinterpret_cast<char *>(&subchunk_size), 4);
  file.write(reinterpret_cast<const char *>(&wav.format.audio_format),
             sizeof wav.format.audio_format);
  file.write(reinterpret_cast<const char *>(&wav.format.num_channels),
             sizeof wav.format.num_channels);
  file.write(reinterpret_cast<const char *>(&wav.format.sample_rate),
             sizeof wav.format.sample_rate);
  file.write(reinterpret_cast<const char *>(&wav.format.byte_rate),
             sizeof wav.format.byte_rate);
  file.write(reinterpret_cast<const char *>(&wav.format.block_align),
             sizeof wav.format.block_align);
  file.write(reinterpret_cast<const char *>(&wav.format.bits_per_sample),
             sizeof wav.format.bits_per_sample);
